...
export default function Case() {
  ...
  return <SafeAreaView style={s.screen}>
    <View style={s.header}><Pressable accessibilityRole="button" accessibilityLabel="Back to Build" style={s.button} onPress={() => router.canGoBack() ? router.back() : router.replace('/build')}><Text style={s.link}>← Build</Text></Pressable><Text style={s.kicker}>STACK / CASE{demoCount !== null ? ' · DEMO' : ''}</Text></View>
    <Text style={s.title}>The weeks you built.</Text><Text style={s.intro}>Every session, kept inside.</Text>
    <FlatList data={entries} numColumns={2} keyExtractor={(entry) => entry.id} initialNumToRender={8} maxToRenderPerBatch={8} windowSize={5} contentContainerStyle={s.grid} columnWrapperStyle={s.columns}
      ListEmptyComponent={<Text style={s.empty}>Your first completed workout will find its place here.</Text>}
      renderItem={({ item }) => <Pressable accessibilityRole={item.week ? 'button' : undefined} disabled={!item.week} accessibilityLabel={`Week of ${label(item.weekStart)}. ${item.week ? `${countLabel(item.week.metrics.workouts, 'workout')}, ${countLabel(item.week.metrics.records, 'record')}. Unpack week.` : 'No logged workouts.'}`} style={s.niche} onPress={() => { setSelected(item.week!.id); setPieceId(null); setFailed(false); }}>
...
const s = StyleSheet.create({
  ...
  button: { minHeight: 44, justifyContent: 'center', paddingHorizontal: 8 }, link: { color: c.bone, fontFamily: f.uiSemiBold, fontSize: 14 }, kicker: { color: c.ash, fontFamily: f.mono, fontSize: 10 },
